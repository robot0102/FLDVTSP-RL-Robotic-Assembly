# -*- coding: utf-8 -*-
"""
# @Time    : 23/07/19 9:10 PM
# @Author  : ZHIMIN HOU
# @FileName: assembly_main.py
# @Software: PyCharm
...
class GPSMain(object):
    ...
    def run(self, itr_load=None):
        ...
        try:
            ...
            episodes_rewards = np.zeros((self._hyperparams['iterations'], self._conditions), dtype=np.float32)
            episodes_times = np.zeros((self._hyperparams['iterations'], self._conditions), dtype=np.float32)
            for itr in range(itr_start, self._hyperparams['iterations']):
                ...
                pol_sample_lists, episodes_rewards[itr, :], episodes_times[itr, :] = self._take_policy_samples()
                # self._log_data(itr, traj_sample_lists, pol_sample_lists)

                np.save(self._data_files_dir + 'episodes_reward.npy', copy.copy(episodes_rewards))
                np.save(self._data_files_dir + 'episodes_assembly.npy', copy.copy(episodes_times))
    ...
    def _take_policy_samples(self, N=None):
        ...
        episode_times = [None for _ in range(len(self._test_idx))]
        episode_rewards = [None for _ in range(len(self._test_idx))]

        # Since this isn't noisy, just take one sample.
        # TODO: Make this noisy? Add hyperparam?
        # TODO: Take at all conditions for GUI?
        for cond in range(len(self._test_idx)):
            pol_samples[cond][0], episode_rewards[cond], episode_times[cond] = self.agent.test_sample(
                self.algorithm.policy_opt.policy, self._test_idx[cond],
                verbose=verbose, save=False, noisy=False, length=200)
        return [SampleList(samples) for samples in pol_samples], episode_rewards, episode_times
